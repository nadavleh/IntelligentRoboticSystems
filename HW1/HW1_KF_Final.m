...
figure;
plot(xx,pdf(prediction,xx'),'g',xx,pdf(observation,xx'),'b',xx,pdf(posterior,xx'),'r',[x_real(1,Ind+1) x_real(1,Ind+1)],[0 1],'--k');
grid on; title(['time: ' num2str(i) '[sec]']); xlabel('x[m]'); ylabel('density');
legend('prediction PDF','observation PDF','correction PDF','real position');
end
